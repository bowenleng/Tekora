...
public abstract class AbstractPipe extends AbstractConnector {
    ...
    protected static final VoxelShape CORE = box(5, 5, 5, 9, 9, 9);
    ...
    public void changeBlockState(BlockState state, Direction direction) {
        ...
        switch (dir) {
            case UP -> state.setValue(UP, values[state.getValue(UP).ordinal() % values.length]);
            case DOWN -> state.setValue(DOWN, values[state.getValue(DOWN).ordinal() % values.length]);
            case NORTH -> state.setValue(NORTH, values[state.getValue(NORTH).ordinal() % values.length]);
            case SOUTH -> state.setValue(SOUTH, values[state.getValue(SOUTH).ordinal() % values.length]);
            case EAST -> state.setValue(EAST, values[state.getValue(EAST).ordinal() % values.length]);
            case WEST -> state.setValue(WEST, values[state.getValue(WEST).ordinal() % values.length]);
        }
    }
}
