...
public abstract class AbstractCable extends AbstractConnector {
    protected static final VoxelShape CORE = box(6, 6, 6, 8, 8, 8);
    ...
    public void changeBlockState(BlockState state, Direction direction) {
        ...
        switch (dir) {
            case UP -> state.setValue(UP, values[state.getValue(UP).ordinal() % 2]);
            case DOWN -> state.setValue(DOWN, values[state.getValue(DOWN).ordinal() % 2]);
            case NORTH -> state.setValue(NORTH, values[state.getValue(NORTH).ordinal() % 2]);
            case SOUTH -> state.setValue(SOUTH, values[state.getValue(SOUTH).ordinal() % 2]);
            case EAST -> state.setValue(EAST, values[state.getValue(EAST).ordinal() % 2]);
            case WEST -> state.setValue(WEST, values[state.getValue(WEST).ordinal() % 2]);
        }
    }
}
